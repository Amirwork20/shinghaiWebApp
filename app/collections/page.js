...
export default function CollectionsPage() {
  ...
  const { getCurrentCategoryName, activeCategory, handleCategoryChange } = useCategory()
  const { 
    sortBy, 
    setSortBy, 
    season, 
  ...
  useEffect(() => {
    const fetchFabrics = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/web/fabrics`)
  ...
  const findParentSubCategory = () => {
    ...
    for (const main of categories) {
      for (const sub of main.sub_categories || []) {
        const found = sub.categories?.find(cat => cat._id === activeCategory.category.id)
        if (found) {
          return {
            mainCategory: main,
            subCategory: sub,
  ...
  const mainCategory = categories.find(cat => cat._id === activeCategory.mainCategory.id) 
  ...
  const selectedSubSubCategory = activeCategory.category.id 
    ? (subCategory?.categories?.find(cat => cat._id === activeCategory.category.id) 
      || parentCategories?.category)
    : null

  // If there's a main category selected but no subcategory, select the first one
  useEffect(() => {
    if (mainCategory?._id && !activeCategory.subCategory.id && mainCategory.sub_categories?.[0]) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 mb-4 sm:mb-8">
        <div className="relative h-[200px] sm:h-[300px] w-full">
          <img
            src={mainCategory.image_url || defaultMainCategory.image_url || null}
            alt={(mainCategory.category_name || defaultMainCategory.category_name || '')}
        ...
        {(subSubCategories.length > 0 || defaultSubSubCategories.length > 0) && (
          <div className="flex flex-wrap place-items-center justify-center gap-4 max-w-[85%] sm:max-w-[80%] md:max-w-[75%] lg:max-w-[70%] mx-auto">
            {(subSubCategories.length > 0 ? subSubCategories : defaultSubSubCategories)
              .filter(category => 
              ...
              .map((category) => (
                <Link
                  href="/collections"
                  key={category._id}
                  onClick={(e) => handleCategoryClick(e, mainCategory, subCategory, category)}
                  className={`flex flex-col items-center justify-center text-center w-[calc(50%-8px)] sm:w-[calc(33.333%-11px)] md:w-[calc(25%-12px)] lg:w-[calc(20%-13px)] ${
                    category._id === activeCategory.category.id ? 'opacity-50' : ''
                  }`}
                >
              ...
              <div className="absolute right-0 top-0 h-full w-[300px] bg-white p-6 overflow-y-auto">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <Select value={fabric || 'all'} onValueChange={handleFabricChange}>
                      <SelectTrigger className="bg-white">
